...
import { paletteOneLight } from './paletteOne/paletteOneLight';
import { paletteOneDark } from './paletteOne/paletteOneDark';
...
const Palette = (presetColor: string, mode: 'light' | 'dark') => {
  ...
  if (presetColor === 'theme1') {
    colors = mode === 'dark' ? paletteOneDark : paletteOneLight;
  } else {
